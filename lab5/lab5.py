...
# перевіряємо наявність завантажених файлів
    existing_files = [f for f in os.listdir(DATA_DIR) if f.endswith(".csv")]
    if len(existing_files) < 27:
        progress_text = "Дані не знайдено. Завантаження."
        my_bar = st.progress(0, text=progress_text)
        
        for i, province_id in enumerate(range(1, 28)):
# перевіряємо, чи є вже файл для цієї області
            ...
            my_bar.progress((i + 1) / 27, text=f"Завантаження даних. {int(((i+1)/27)*100)}%")
...
if df.empty:
    st.error("Не вдалося завантажити дані. Перевірте з'єднання з інтернетом.")
    st.stop()
